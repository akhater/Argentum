...
| ⬜ | **f32 all the way in, not just out** | The input is still uploaded as `Rgba16Float`, so the export is bounded at about 11 significant bits by the *upload* and the f16 intermediates rather than by the output. A 14-bit sensor holds 13 bits in its top stop, so the brightest two are thinned for rendering. **Not** before highlight recovery, which an earlier version of this row claimed: `raw_processing.rs:77` runs `mods::decode::on_raw_decoded` - and `highlights::recover` inside it - on the Bayer data, before anything reaches a texture. Reconstruction sees full precision; only its rendering is thinned. Blocked on the flare pass, which samples the input through a `Filtering` sampler (`flare.wgsl`, `textureSampleLevel`); a filtered `Rgba32Float` needs `FLOAT32_FILTERABLE`, which is not requested. Requesting it conditionally would make the same export produce different precision on different machines, so the real options are to require the feature, or to give the flare pass its own f16 copy of the input | 1 day |
