...
| ⬜ | **Memory usage** | Nobody has measured the high-water mark on a large RAW, and the pipeline has been adding full-resolution copies rather than removing them: the upload, the patched/warped preview, and now an `rgba32float` export target that costs four bytes a channel where the preview costs two. Going f32 all the way in doubles the input too. The work is to measure first - peak resident set across decode, edit and export on a 45MP file - and only then decide what to drop, reuse or stream, because every candidate here is a guess until something is on a graph | measure: 1 day |
...
| | What | Why | Effort |
|---|---|---|---|
| 🟨 | **16-bit TIFF export** | Shipped in `26.37.20`, not finished. The render writes into an `rgba32float` storage texture for TIFF and quantises once at the encode; the encoder was already correct - it was being handed 8-bit data. See `mods/export_precision.rs` and the `high-precision-export` registry entry. It stays in progress because the file holds 16 bits the pipeline cannot yet fill: the row below bounds it at about 11 significant bits, so the header is honest about the container and optimistic about the content | rest below |
| ⬜ | **Export precision selector, step 1: 8-bit TIFF and the control** | **What #1466 adds, read from the diff.** Frontend: a `Dropdown` (8-bit / 16-bit) inside `ExportPanel.tsx` under `fileFormat === FileFormats.Tiff`, ~15 lines; `tiffBitDepth` state, preset load/save and return in `useExportSettings.ts`; `type TiffBitDepth = 8 \| 16` and a preset field in `ExportImportProperties.tsx`; a default in `useExternalEditSession.ts`; three strings in all thirteen locale files. Rust: a `TiffBitDepth` enum (serde `try_from = "u8"`, rejects anything but 8 and 16, `#[serde(default)]` = 16 so old presets keep working), a field on `ExportSettings`, `render_output_precision()`, **a fourth parameter on `encode_image_to_bytes`** threaded through `process_image_for_export`, `export_masks_for_image`, both `estimate_export_sizes` sites, `run_headless_export`, `launch_request.rs` and `lib.rs`. **Cannot be taken as-is:** two states only by design, logic in six of their files, and that threaded encoder parameter is the exact counter-example CLAUDE.md names. **Argentum-shaped:** `Precision` stays the single value the render reads, gaining `for_export(path)` and `tiff_depth()`. The mode persists in `argentum-processing.json` beside `highlightRecovery` (`mods/highlights.rs:44-57` is the pattern), through `ag('get_export_precision')` / `ag('set_export_precision')`. The control is `src/argentum/ExportPrecision.tsx`, shown only for TIFF, tagged once into `ExportPanel.tsx` — import plus tag, 2 hooks, new `ANCHORS` entry, precedent `Color.tsx`. Strings in our own namespace; **zero locale files of theirs, zero new Rust anchors.** **8-bit mode** is `Precision::Preview` — the same render a JPEG gets, dither on. The container comes from **their** encoder arm, borrowed as `encode_tiff(image)` in our file and called from their TIFF arm (1 line replaced, rides the existing import, registry `borrows` #1466), which also fixes the pre-existing `.tif` gap: theirs matches `"tif" \| "tiff"`, ours matched `"tiff"` only. **One consequence worth the trouble:** dispatch by image type means `estimate_export_sizes` would quote the 8-bit size for a 16-bit export, so both estimate sites move to `render_for_export` (2 lines of theirs, same pattern as the masks, cheap because estimates run on a downscaled preview). **Not covered, written down:** the mode is not in their export presets and not a headless flag; `run_headless_export` uses the persisted mode, and `--tiff-bit-depth` can be borrowed later. **When #1466 lands:** their dropdown appears beside ours and their `ExportSettings` field arrives unused — one review decision, delete their block as `shadows` or retire ours. **Tests:** an 8-bit TIFF decodes as `Rgb8` and equals `to_rgb8()` of the same Preview render; the mode round-trips through the file; a tree with no mode saved still exports 16-bit, so no existing user's output changes. | 1 day |
